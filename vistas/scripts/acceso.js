...
function listar()
{
	tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/acceso.php?op=listarsucursalesporusuario',
					type : "get",
					dataType : "json",						
					error: function(e){
...
function mostrar(idsucursal)
{
	...
	$.post("../ajax/acceso.php?op=sesionarsucursal",{idsucursal : idsucursal}, function(data, status)
	{

	data = JSON.parse(data);		
		
		console.log(data);
		if(data.idcargo == "1" || data.idcargo == "2"){
			window.location.replace("../vistas/escritorio.php");
			console.log(data.idcargo);
		}
		else{
			window.location.replace("../vistas/ingreso.php");
			console.log(data.idcargo);
		}
...
{
	bootbox.confirm("¿Está Seguro de ir a  la Sucursal?", function(result){
		if(result)
        {
