...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idventa)){
			$rspta=$venta->insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_venta,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio_venta"],$_POST["descuento"],$idcaja);
	...
	case 'listar':
		 ...
		 // listar por ventas de sucursales
		 if($_SESSION["idsucursal"] != "" && ($_SESSION["cargo"]==1 || $_SESSION["cargo"]==2)){
				$rspta=$venta->listarventasporsucursales();
			//Vamos a declarar un array
			$data= Array();
			while ($reg=$rspta->fetch_object()){
				if($reg->tipo_comprobante=='Ticket'){
			...
			if($_SESSION["idsucursal"] != "" && $_SESSION["cargo"]==3){
				$rspta=$venta->listarventasporsucursalesycajeros();
			//Vamos a declarar un array
			$data= Array();
			while ($reg=$rspta->fetch_object()){
				if($reg->tipo_comprobante=='Ticket'){
