...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idingreso)){
			$rspta=$ingreso->insertar($idproveedor,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio_compra"],$_POST["precio_venta"],$idcaja);
	...
	if($_SESSION["idsucursal"] != "" && ($_SESSION["cargo"]==1 || $_SESSION["cargo"]==2)){
		$rspta=$ingreso->listaringresoporsucursales();
 		//Vamos a declarar un array
 		$data= Array();
	...
	if($_SESSION["idsucursal"] != "" && $_SESSION["cargo"]==3){
		$rspta=$ingreso->listaringresoporsucursalesycajero();
 		//Vamos a declarar un array
 		$data= Array();
	...
	case 'selectCaja':
		...
		$caja = new Caja();
		if($_SESSION["cargo"]==1 && $_SESSION["idsucursal"]==""){
		$rspta = $caja->listarActivos();
		echo '<option value="" disabled="true" selected>Selecciona</option>';
		while ($reg = $rspta->fetch_object())
				{
				echo '<option value=' . $reg->idcaja . '>' . $reg->codcaja . '</option>';
				}
			}

		if(($_SESSION["cargo"]==1 || $_SESSION["cargo"]==2) && $_SESSION["idsucursal"]!=""){
				$rspta = $caja->listarActivosporsucursales();
				echo '<option value="" disabled="true" selected>Selecciona</option>';
				while ($reg = $rspta->fetch_object())
						{
						echo '<option value=' . $reg->idcaja . '>' . $reg->codcaja . '</option>';
		...
		if(($_SESSION["cargo"]==3 ) && $_SESSION["idsucursal"]!=""){
				$rspta = $caja->listarActivosporsucursalesycajeros();
				echo '<option value="" disabled="true" selected>Selecciona</option>';
				while ($reg = $rspta->fetch_object())
				{
					echo '<option value=' . $reg->idcaja . '>' . $reg->codcaja . '</option>';
