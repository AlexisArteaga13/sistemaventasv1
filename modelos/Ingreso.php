...
Class Ingreso
{
	...
	public function insertar($idproveedor,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$idarticulo,$cantidad,$precio_compra,$precio_venta,$idcaja)
	{
		$sql="INSERT INTO ingreso (idproveedor,idusuario,tipo_comprobante,serie_comprobante,num_comprobante,fecha_hora,impuesto,total_compra,estado,cajaderegistro)
		VALUES ('$idproveedor','$idusuario','$tipo_comprobante','$serie_comprobante','$num_comprobante','$fecha_hora','$impuesto','$total_compra','Aceptado','$idcaja')";
		...
		{
			$idarticuloarevisar =$idarticulo[$num_elementos];
			$articulo = new Articulo();
			$comprobarimpuesto = $articulo->mostrararticuloarevisar($idarticuloarevisar);
			$estadoimpuesto = implode($comprobarimpuesto);
			if($estadoimpuesto == "1"){
				$precio_venta[$num_elementos]=$precio_venta[$num_elementos]+$precio_venta[$num_elementos]*0.18;
			}
			else{
				$precio_venta[$num_elementos]=$precio_venta[$num_elementos];
			}
	...
	// LISTAR INGRESOS POR SUCURSALES
	public function listaringresoporsucursales()
	{
		$sql="SELECT i.idingreso,DATE(i.fecha_hora) as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,u.nombre as usuario,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.impuesto,i.estado FROM ingreso i INNER JOIN persona p ON i.idproveedor=p.idpersona INNER JOIN usuario u ON i.idusuario=u.idusuario INNER join caja ca on i.cajaderegistro=ca.idcaja WHERE ca.id_sucursal='".$_SESSION["idsucursal"]."' ORDER BY i.idingreso desc";
		return ejecutarConsulta($sql);		
	}
	// LISTAR INGRESOS POR SUCURSALESYCAJERO
	public function listaringresoporsucursalesycajero()
	{
		$sql="SELECT i.idingreso,DATE(i.fecha_hora) as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,u.nombre as usuario,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.impuesto,i.estado FROM ingreso i INNER JOIN persona p ON i.idproveedor=p.idpersona INNER JOIN usuario u ON i.idusuario=u.idusuario INNER join caja ca on i.cajaderegistro=ca.idcaja WHERE ca.id_sucursal='".$_SESSION['idsucursal']."' AND ca.cajero='".$_SESSION["idusuario"]."' ORDER BY i.idingreso desc";
		return ejecutarConsulta($sql);		
	}
